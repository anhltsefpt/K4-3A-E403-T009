...
- **Phân tích lượt 1 (vì sao 11 ca "sai"):** con số 66% bị kéo xuống bởi **lỗi định dạng**, không phải AI hiểu sai.
  ...
  - **→ Sửa để tốt hơn (đã làm ở lượt 1b & 2, có số chứng minh):** (a) **chấm deadline theo NGÀY** thay vì so chuỗi → gỡ 7 ca bị trừ oan do format (**66%→81%**); (b) **nhấn ngưỡng ≤2 ngày + liệt kê luật skip** trong prompt → hết over-eager: do_now 2/3→3/3, soon 3/5→4/5, skip 19→21.
- **Phân tích lượt 2 (đánh đổi, KHÔNG phải thắng sạch):**
  ...
  - **→ Sửa để tốt hơn (đã làm ở lượt 3, có số chứng minh):** thêm **luật đối chiếu 2 tin cùng chủ đề** vào prompt (thay vì xử từng tin độc lập) → confirm ⚪ hồi **0/2→2/2**, tổng **88%→91%** — vừa giữ được phần tốt của lượt 2 (do_now/soon/skip) vừa cứu lại ngôi sao sản phẩm.
- **Phân tích lượt 3 (CHỐT — 91%):** thêm luật đối chiếu mâu thuẫn vào prompt (giữ matcher-theo-ngày + phần tốt của lượt 2).
  ...
  - **→ Sửa để tốt hơn (đã làm ở lượt 4, có bằng chứng):** cố định **`temperature=0`** — đây không phải để tăng %, mà để **tái lập**: trước đó số nhảy 88–91% giữa các lần, 91% chỉ là **một lần chạy may**. Sau khi cố định, số ổn định về **88%** (đánh đổi 91%→88% lấy tính trung thực + tái lập). Đây là lý do lượt 4 mới là bản nộp, không phải lượt 3.
- **Phân tích lượt 4 (CHỐT — 88%, tái lập được):** phát hiện khi chạy bot LIVE trên server thật: AI nhầm `sent_at` (giờ gửi tin) thành deadline → gán cả tin tán gẫu ("oke nhé", "chia gì cơ") vào 🟡 với hạn = giờ gửi. Sửa:
  ...
  - **→ Sửa để tốt hơn:** lượt 4 là **bản chốt** nên **không áp thêm fix nào** (quality bar đã khóa). 4 ca sai còn lại được **chấp nhận có chủ ý** vì đều "sai an toàn": biên giới do_now/soon (M01982, SYN03) hoặc over-confirm (M07416) — **không** phải chốt sai deadline. *Hướng cải tiến (chưa kiểm chứng, không tính vào số nộp):* thêm vài **ví dụ biên ≤2 ngày** (few-shot) vào prompt để bớt lệch M01982/SYN03 — chỉ ghi làm ghi chú, chưa chạy đo nên không tuyên bố kết quả.
